...
const Dashboard = () => {
  ...
  const retailerData = {
    ...
    recentTransactions: [
      { date: '2025-01-15', type: 'Token Sale', amount: 5000, investor: 'Community Pool' },
      { date: '2025-01-10', type: 'Profit Distribution', amount: -3200, investor: 'All Holders' },
      { date: '2025-01-05', type: 'Token Sale', amount: 7500, investor: 'Individual Investors' }
    ]
  };
      ...
      <div className="container mx-auto px-4">
        ...
        <div className="text-center mb-8">
          <div className="inline-flex items-center px-4 py-2 rounded-full glass border border-primary/20 glow-primary mb-6">
            ...
            <span className="text-sm font-medium text-primary">Unified Analytics</span>
          </div>
          ...
          <div className="flex justify-center mb-6">
            <div className="glass rounded-lg p-1 border border-primary/20">
              <Button
                ...
                className={viewMode === 'retailer' ? 'bg-gradient-primary' : 'hover:bg-muted/20'}
              >
              ...
              <Button
                ...
                className={viewMode === 'investor' ? 'bg-gradient-secondary' : 'hover:bg-muted/20'}
              >
            ...
            <div className="grid lg:grid-cols-2 gap-8">
                  ...
                  <div className="space-y-4">
                    {retailerData.forecasts.map((forecast, index) => (
                      <div key={index} className="flex items-center justify-between p-4 glass rounded-lg">
                        <div>
                          <div className="font-semibold text-foreground">{forecast.month}</div>
                        ...
                        <div className="text-right">
                          {forecast.accuracy ? (
                            <div className="text-lg font-bold text-success">{forecast.accuracy}%</div>
                          ) : (
                            <div className="text-lg font-bold text-accent">Pending</div>
                          )}
              ...
              <Card className="glass border-accent/20">
                <CardHeader>
                  ...
                  <CardDescription>Token sales and distributions</CardDescription>
                </CardHeader>
                <CardContent>
                  <div className="space-y-4">
                    {retailerData.recentTransactions.map((tx, index) => (
                      <div key={index} className="flex items-center justify-between p-4 glass rounded-lg">
                        <div>
                          ...
                          <div className="text-sm text-muted-foreground">{tx.investor}</div>
                          <div className="text-xs text-muted-foreground">{tx.date}</div>
                        </div>
                        <div className={`text-lg font-bold ${tx.amount > 0 ? 'text-success' : 'text-primary'}`}>
            ...
            <div className="grid md:grid-cols-4 gap-6">
              ...
              <Card className="glass border-secondary/20 hover-lift">
                <CardContent className="p-6 text-center">
                  ...
                  <div className="text-sm text-muted-foreground">Profits Received</div>
                </CardContent>
              </Card>
            </div>
                ...
                <CardContent>
                  <div className="space-y-4">
                    {investorData.ownedTokens.map((token, index) => (
                      <div key={index} className="p-4 glass rounded-lg hover-lift">
                        <div className="flex justify-between items-start mb-2">
                          <div className="font-semibold text-foreground">{token.business}</div>
                          <div className="text-lg font-bold text-success">+{token.roi}%</div>
                        </div>
                        <div className="grid grid-cols-2 gap-4 text-sm">
                          <div>
                            <div className="text-muted-foreground">Tokens Owned</div>
                            <div className="font-semibold">{token.tokens}</div>
                          </div>
                          <div>
                            <div className="text-muted-foreground">Current Value</div>
                            <div className="font-semibold">RM {token.value.toLocaleString()}</div>
                          </div>
                        </div>
                      </div>
                    ))}
                  ...
                  <div className="space-y-4">
                    {investorData.profitHistory.map((profit, index) => (
                      <div key={index} className="flex items-center justify-between p-4 glass rounded-lg hover-lift">
                        <div>
                          <div className="font-semibold text-foreground">{profit.business}</div>
                          <div className="text-xs text-muted-foreground">{profit.date}</div>
                        </div>
            ...
            <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
              <div>
                ...
                <p className="text-sm text-muted-foreground">
                  Download detailed analytics and transaction history for your records.
                </p>
              </div>
              <div className="flex gap-2">
                ...
                <Button variant="outline" className="glass border-accent/30 hover:bg-accent/10">
                  <Download className="mr-2 h-4 w-4" />
                  Export PDF
                </Button>
              </div>
