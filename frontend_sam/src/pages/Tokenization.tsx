...
const Tokenization = () => {
  ...
  const handleTokenization = async () => {
    ...
    if (!formData.businessName || !formData.description || !formData.revenueEstimate) {
      setTransactionMessage("Please fill in all required fields");
      return;
    }
    ...
    try {
      setTransactionMessage("🔗 Creating tokens on blockchain...");
      
      // Calculate token details
      const totalTokens = calculateTokens();
      const tokenValue = totalTokens * 100; // 100 RM per token
      
      // Create new token entry
      const newToken = {
        id: availableTokens.length + 1,
        business: formData.businessName,
        tokens: totalTokens,
        price: 100,
        totalValue: tokenValue,
        roi: `${(Math.random() * 10 + 5).toFixed(1)}%`,
        sector: "Business",
      ...
      setAvailableTokens(prev => [newToken, ...prev]);
      setIsTokenized(true);
      setTransactionMessage(`✅ Successfully created ${totalTokens} tokens worth RM ${tokenValue.toLocaleString()}!`);
      ...
      setTransactionMessage(`❌ Tokenization failed: ${error.message}`);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleBuyTokens = async (tokenId: number) => {
    ...
    if (parseFloat(buyAmount) > token.tokens) {
      setTransactionMessage(`Only ${token.tokens} tokens available`);
      return;
    }
    ...
    try {
      const totalCost = parseFloat(buyAmount) * token.price;
      const ethAmount = totalCost * 0.0003; // Convert RM to ETH (demo rate: 1 ETH ≈ 3333 RM)

      setTransactionMessage(`💰 Sending ${ethAmount.toFixed(6)} ETH (RM ${totalCost}) to ${token.business}...`);

      // Send real ETH transaction
      await sendETH(token.ownerAddress, ethAmount.toString());
      ...
      setAvailableTokens(prev => 
        prev.map(t => 
          t.id === tokenId 
            ? { ...t, tokens: t.tokens - parseFloat(buyAmount) }
            : t
        )
      );

      setTransactionMessage(`🎉 Successfully purchased ${buyAmount} tokens from ${token.business}! Transaction hash: ${hash?.slice(0, 10)}...`);
      setBuyAmount("");
      setSelectedToken(null);

    } catch (error) {
      console.error('Purchase failed:', error);
      setTransactionMessage(`❌ Purchase failed: ${error.message}`);
    } finally {
      setIsBuying(false);
    }
  };
