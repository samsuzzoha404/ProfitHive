...
class ExternalDataService {
  constructor() {
    ...
    this.weatherApiKey =
      process.env.WEATHER_API_KEY || "1efecea6037395ce7bd137b9113cbe3a";
  ...
  async getWeatherImpact() {
    try {
      ...
      const impactScore = this.calculateWeatherImpactScore(
        temp,
        humidity,
        condition
      );
      ...
      console.log(
        `✅ Weather data retrieved: ${temp}°C, ${condition}, Impact: ${impactScore}`
      );
  ...
  async getTransportImpact() {
    try {
      ...
      const busAvailability = this.calculateBusAvailability(
        currentHour,
        currentDay,
        transportData
      );
      const trainFrequency = this.calculateTrainFrequency(
        currentHour,
        currentDay,
        transportData
      );
      ...
      const impactScore = this.calculateTransportImpactScore(
        busAvailability,
        trainFrequency,
        congestionLevel
      );
      ...
      console.log(
        `✅ Transport data analyzed: Bus ${busAvailability}%, Train ${trainFrequency}%, Congestion ${congestionLevel}%, Impact: ${impactScore}`
      );
      ...
      return {
        ...
        dataSource: transportData.dataSource || {
          provider: "simulated",
          isRealData: false,
        },
  ...
  async loadCyberjayanTransportData() {
    try {
      ...
      const pythonServicePath = path.join(
        ...
        "..",
        "python",
        "kaggle_transport_service.py"
      );

      return new Promise((resolve, reject) => {
        exec(
          `python "${pythonServicePath}" --action=fetch --format=json`,
          {
          ...
          (error, stdout, stderr) => {
            if (error) {
              console.error(
                "❌ Kaggle transport service error:",
                error.message
              );
            ...
            try {
              ...
              console.log(
                `   📊 Data Summary: Bus ${kaggleData.bus_availability}%, Congestion ${kaggleData.congestion_level}%`
              );
              ...
              console.error(
                "❌ Failed to parse Kaggle service output:",
                parseError.message
              );
      ...
      console.error(
        "❌ Error loading Kaggle transportation data:",
        error.message
      );
  ...
  transformKaggleData(kaggleData) {
    try {
      ...
      return {
        ...
        peakHours: kaggleData.peak_patterns?.all_peaks || [
          7, 8, 9, 12, 13, 17, 18, 19,
        ],
        realTimeMetrics: {
        ...
        dataSource: {
          ...
          dataset:
            kaggleData.dataset ||
            "shahmirvarqha/transportation-in-cyberjaya-malaysia",
  ...
  calculateBusAvailability(hour, day, transportData) {
    ...
    if (transportData.busAvailability !== undefined) {
      console.log(
        `🚌 Using real Kaggle bus availability data: ${transportData.busAvailability}%`
      );
  ...
  calculateTrainFrequency(hour, day, transportData) {
    ...
    if (
      transportData.realTimeMetrics &&
      transportData.realTimeMetrics.rideServiceLevel !== undefined
    ) {
      console.log(
        `🚊 Using real Kaggle ride service data as train proxy: ${transportData.realTimeMetrics.rideServiceLevel}%`
      );
  ...
  calculateCongestionLevel(hour, day, transportData) {
    ...
    if (transportData.congestionLevel !== undefined) {
      console.log(
        `🚗 Using real Kaggle congestion data: ${transportData.congestionLevel}%`
      );
    ...
    try {
      ...
      return {
        ...
        combinedScore: Math.round(
          (weatherImpact.impactScore + transportImpact.impactScore) / 2
        ),
        timestamp: new Date().toISOString(),
      };
    } catch (error) {
