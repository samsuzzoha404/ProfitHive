...
app.post("/api/forecast", async (req, res) => {
  ...
  try {
    ...
    if (!validation.valid) {
      ...
      return res
        .status(400)
        .json(validationService.formatValidationErrors(validation.errors));
    }
    ...
    if (!saveResult.success) {
      console.log(
        "Warning: Failed to save forecast results:",
        saveResult.error
      );
...
const server = app.listen(PORT, () => {
  ...
  console.log("\n📋 Available Services:");
  console.log(
    "  • Enhanced AI Statistical Algorithms (Advanced Pattern Recognition)"
  );
