...
class KaggleTransportService:
    ...
    def _get_default_ride_sharing(self, hour, day):
        ...
        """Analyze bus route data from Kaggle dataset"""
        try:
            # Adapt this based on actual dataset structure
            # Example assuming columns like: route_id, frequency, reliability, schedule
            
            if 'route_id' in df.columns:
                bus_data = df[df['transport_type'] == 'bus'] if 'transport_type' in df.columns else df
                
                routes = []
                for route_id in bus_data['route_id'].unique() if 'route_id' in bus_data.columns else ['T101', 'T102', 'T103']:
                    route_data = bus_data[bus_data['route_id'] == route_id] if 'route_id' in bus_data.columns else bus_data.sample(1)
                    
                    routes.append({
                        'route': route_id,
                        'frequency': route_data['frequency'].mean() if 'frequency' in route_data.columns else 15 + np.random.randint(0, 15),
                        'reliability': route_data['reliability'].mean() if 'reliability' in route_data.columns else 80 + np.random.randint(0, 15),
                        'current_service_level': self._get_service_level_by_time(hour, day)
                    })
                ...
                return [
                    {'route': 'T101', 'frequency': 15, 'reliability': 85, 'current_service_level': 90},
                    {'route': 'T102', 'frequency': 20, 'reliability': 80, 'current_service_level': 85},
                    {'route': 'T103', 'frequency': 30, 'reliability': 90, 'current_service_level': 88}
                ]
                
        except Exception as e:
            logger.warning(f"Error analyzing bus routes: {e}")
            return []
    
    def _analyze_train_frequency(self, df, hour, day):
        """Analyze train service data"""
        try:
            # Look for train/rail data in the dataset
            train_data = df[df['transport_type'] == 'rail'] if 'transport_type' in df.columns else df
            
            if len(train_data) > 0:
                return {
                    'klia_ekspres': {
                        'frequency': train_data['frequency'].mean() if 'frequency' in train_data.columns else 20,
                        'reliability': train_data['reliability'].mean() if 'reliability' in train_data.columns else 95,
                        'service_level': 95 if self._is_peak_hour(hour) else 90
                    },
                    'klia_transit': {
                        'frequency': train_data['frequency'].mean() if 'frequency' in train_data.columns else 30,
                        'reliability': train_data['reliability'].mean() if 'reliability' in train_data.columns else 92,
                        'service_level': 90 if self._is_peak_hour(hour) else 85
                    }
                }
            else:
                # Default train data
                return {
                    'klia_ekspres': {'frequency': 20, 'reliability': 95, 'service_level': 90},
                    'klia_transit': {'frequency': 30, 'reliability': 92, 'service_level': 85}
                }
                
        except Exception as e:
            logger.warning(f"Error analyzing train data: {e}")
            return {}
    
    def _analyze_congestion(self, df, hour, day):
        """Analyze traffic congestion patterns"""
        try:
            if 'congestion_level' in df.columns or 'traffic_density' in df.columns:
                congestion_col = 'congestion_level' if 'congestion_level' in df.columns else 'traffic_density'
                
                # Get hourly congestion patterns
                hourly_congestion = {}
                if 'hour' in df.columns:
                    for h in range(24):
                        hour_data = df[df['hour'] == h]
                        if len(hour_data) > 0:
                            hourly_congestion[h] = hour_data[congestion_col].mean()
                
                return {
                    'hourly_patterns': hourly_congestion,
                    'current_level': hourly_congestion.get(hour, 35),
                    'peak_congestion_hours': [7, 8, 17, 18, 19]
                }
            else:
                # Generate realistic congestion patterns
                return {
                    'hourly_patterns': {h: 20 + (30 if h in [7,8,17,18,19] else 0) + np.random.randint(-5, 5) for h in range(24)},
                    'current_level': 45 if hour in [7,8,17,18,19] else 25,
                    'peak_congestion_hours': [7, 8, 17, 18, 19]
                }
                
        except Exception as e:
            logger.warning(f"Error analyzing congestion: {e}")
            return {}
    
    def _analyze_peak_hours(self, df):
        """Identify peak transportation hours"""
        try:
            if 'hour' in df.columns and 'passenger_count' in df.columns:
                hourly_passengers = df.groupby('hour')['passenger_count'].sum()
                peak_threshold = hourly_passengers.quantile(0.75)
                peak_hours = hourly_passengers[hourly_passengers >= peak_threshold].index.tolist()
                ...
                # Default peak patterns for Cyberjaya
                return {
                    'morning_peak': [7, 8, 9],
                    'lunch_peak': [12, 13],
                    'evening_peak': [17, 18, 19],
            ...
            logger.warning(f"Error analyzing peak hours: {e}")
            return {}
    
    def _calculate_bus_availability(self, routes, hour, day):
        """Calculate real-time bus availability percentage"""
        if not routes:
            return 75.0
            
        total_service = sum(route.get('current_service_level', 75) for route in routes)
        avg_service = total_service / len(routes)
        
        # Adjust for time of day
        if self._is_peak_hour(hour):
            avg_service *= 1.1  # 10% boost during peak hours
        elif 22 <= hour or hour <= 6:
            avg_service *= 0.7  # Reduced service at night
            
        return min(100, max(0, avg_service))
    
    def _calculate_train_service(self, train_data, hour, day):
        """Calculate train service level"""
        if not train_data:
            return 80.0
            
        service_levels = [train['service_level'] for train in train_data.values()]
        avg_service = sum(service_levels) / len(service_levels)
        
        return min(100, max(0, avg_service))
    
    def _calculate_current_congestion(self, congestion_data, hour, day):
        """Calculate current congestion level"""
        if not congestion_data:
            return 35.0
            
        hourly_patterns = congestion_data.get('hourly_patterns', {})
        current_congestion = hourly_patterns.get(hour, congestion_data.get('current_level', 35))
        
        # Weekend adjustment (less congestion)
        if day >= 5:  # Saturday or Sunday
            current_congestion *= 0.7
            
        return min(100, max(0, current_congestion))
    
    def _calculate_transport_score(self, bus_avail, train_freq, congestion):
        """Calculate overall transportation impact score (0-100)"""
        # Higher bus/train availability = positive impact
        # Higher congestion = negative impact
        positive_impact = (bus_avail + train_freq) / 2
        negative_impact = congestion
        
        # Weighted calculation: 60% positive factors, 40% negative factors  
        score = (positive_impact * 0.6) + ((100 - negative_impact) * 0.4)
        
        return min(100, max(0, score))
    
    def _is_peak_hour(self, hour, peak_data=None):
        """Check if current hour is a peak transportation hour"""
        if peak_data:
            return hour in peak_data.get('all_peaks', [])
        return hour in [7, 8, 9, 12, 13, 17, 18, 19]
    
    def _get_service_level_by_time(self, hour, day):
        """Get service level adjustment by time"""
        if self._is_peak_hour(hour):
            return 90 + np.random.randint(0, 10)
        elif 22 <= hour or hour <= 6:
            return 60 + np.random.randint(0, 15)
        else:
            return 80 + np.random.randint(0, 10)
    ...
    def _get_fallback_data(self):
        ...
        return {
            'bus_availability': 85.0 if self._is_peak_hour(current_hour) else 75.0,
            'train_frequency': 90.0 if self._is_peak_hour(current_hour) else 80.0,
            'congestion_level': 55.0 if self._is_peak_hour(current_hour) else 35.0,
            'impact_score': 75.0,
            'peak_hour': self._is_peak_hour(current_hour),
            'real_data': False,
            'source': 'fallback',
            'timestamp': datetime.now().isoformat(),
